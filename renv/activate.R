local({
  ...
  bootstrap <- function(version, library) {
    ...
    withCallingHandlers(
      ...
      error = function(err) {
        catf("FAILED")
        rlang::abort("failed to download:\n", conditionMessage(err))
      }
    )
    catf("OK")
  ...
  renv_bootstrap_download <- function(version) {
    ...
    rlang::abort("All download methods failed")
  }
  ...
  renv_bootstrap_download_custom_headers <- function(url) {
    ...
    if (!ok) {
      rlang::abort("invocation of 'renv.download.headers' did not return a named character vector")
    }
  ...
  renv_bootstrap_download_cran_latest_find <- function(version) {
    ...
    rlang::abort(sprintf(fmt, version))
  }
